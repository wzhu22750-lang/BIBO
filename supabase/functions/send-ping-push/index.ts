...
Deno.serve(async (request) => {
  ...
  console.log(`[send-ping-push] Ping ${verifiedRecord.id} -> Partner ${partner} (${targets.length} targets)`)
  let access: string
  try {
  ...
  for (const device of targets) {
    ...
    console.log(`[send-ping-push] Token ${token.slice(0, 10)}... status: ${result.status}, ok: ${result.ok}`)
    if (result.ok) {
      sent++
      continue
    }

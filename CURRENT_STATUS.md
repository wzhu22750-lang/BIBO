...
>
> 迁移与线上环境的最新状态以 [DATABASE.md](DATABASE.md) 为准，本文件不重复维护。
...
| **Supabase 数据库与 RLS**    | 95%    | 初始迁移 + 增量迁移全部就绪（迁移清单与线上状态见 DATABASE.md）；线上已应用加固与行锁保护      |
...
| **生产发布就绪度 (Release)** | 待验收 | 缺少正式 release keystore 签名环境变量，暂未进行物理双机个推验收                               |

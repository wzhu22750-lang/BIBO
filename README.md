...
> 网页关闭、手机锁屏或系统挂起时，当前版本不能保证提醒送达。远程 FCM Function 已部署但尚未完成真实设备送达验收；仍没有已读回执或端到端加密。Android 本机通知和本机定时提醒不等同于远程推送。演示中的小橘、小桃和三张照片是示例，不是真实伴侣数据。
...
1. 创建一个 Supabase 项目。
2. 在 SQL Editor 或 Supabase CLI 按文件名顺序执行 `supabase/migrations/202609070001_initial.sql` 和 `supabase/migrations/202609080001_*.sql` 至 `202609080020_*.sql`。每个迁移只执行一次；不要把新增迁移单独跳过。
...
- 仓库已提供 FCM 服务端发送模板：`send-ping-push`（Ping）与 `send-message-push`（聊天消息）均只接受带 `x-bibo-webhook-secret` 的数据库 Webhook，服务端读取伴侣的 Android token 后走 FCM HTTP v1；前台消息由 Supabase Realtime 更新页面；服务端 Push 不使用隐藏的前台时间抑制窗口，消息是否显示系统横幅由 Android/Capacitor 通知链路决定。两个函数已部署到默认 Supabase 项目 version 7，但 Database Webhook 尚未完成触发验收；仓库不包含 Firebase service account，token 登记不等于远程消息已发送或送达。请勿用于紧急联络。
...
当前已生成 Android 工程，并提供 `npm run android:sync` / `npm run android:build`。工程使用 Capacitor 8、Kotlin 原生插件和 `src/native/index.ts` Web fallback；生产 APK 默认捆绑本地 `dist`，不再信任可变的远程 `server.url`。本地可构建调试 APK，但通知、AlarmManager、UsageStats 和厂商后台策略仍需真实设备验收。
...
随后在 Android Studio 配置应用 ID、签名、SDK 和 APK 输出。`android:release:check` 会在缺少 Firebase client、通知图标或生产远程站点配置时明确失败。这只是进入原生开发的步骤，不是“现在已能登录并可靠推送”的承诺。上架前还需：
...
- 账号注销代码位于 `supabase/functions/delete-account`；默认 BIBO 项目已部署 version 1，并配置 `SUPABASE_SERVICE_ROLE_KEY` 等服务端 secrets，该密钥绝不能进入 Vite 或 APK。你切换到自己的 Supabase 项目时仍需单独部署函数和迁移。注销会匿名化共享记录、清理该账号上传的 Storage 文件并删除 Auth 用户，失败阶段会明确返回，未部署函数时客户端不会假装成功。部署模板见该目录 README。
...
- 照片支持上传、文字/发生日期/事件/聊天关联、编辑和上传者确认删除；统一通过固定 ID 的幂等上传队列，在线和离线使用同一条路径。不自动压缩、不移除 EXIF、不支持 HEIC。真实单张上限 5 MB，私有链接有效期 1 小时并会定时刷新。
- 照片上传成功、元数据写入失败时，会尝试清理本人的孤立文件；清理失败会显示具体路径，不冒充上传成功。已下载照片使用按空间/路径/创建时间生成的 IndexedDB 持久缓存，64 MB 总上限、5 MB 单文件上限和 LRU 淘汰；注销、退出、解绑、删除照片时清理。离线快照恢复后若本机有 Blob 缓存仍可显示照片。
...
- Android 原生层已包含系统通知、非精确定时提醒、UsageStats、本机 Focus 查询和可选 Push token 登记；日期/时间输入已改为应用内 PixelDatePicker / PixelTimePicker，不调用 OEM 原生 Picker；通知频道使用私密锁屏可见性和专用状态栏图标；`supabase/functions/send-ping-push` 与 `send-message-push` 提供 FCM HTTP v1 服务端发送模板（消息通知走独立渠道 `bibo_messages_v2`，点击经 `#chat?message=<id>` Deep Link 回到对应聊天），但需自行配置 Webhook、Firebase service account 和 secrets；真实发送、跨设备回执和厂商后台可靠性仍未验收；本地仅验证了 Firebase 组件初始化和 Function 鉴权。
...
纪念日页已增加真实恋爱里程碑、多色倒计时卡片、自绘像素小兔 / 小狗及 SVG 图标选择器。手机底栏中间为 BIBU 哔卟按钮；照片墙、专注陪伴和设置通过右侧「更多」进入，桌面仍保留原侧栏。
